...
class NavViewSet(viewsets.ModelViewSet):
    ...
    def listing(self, request):
        ...
        page_size = min(int(request.query_params.get('page_size', 100)), 1000)  # Cap at 1000
        ...
        try:
            ...
            # Use PostgreSQL function for search with proper error handling
            with connection.cursor() as db_cursor:
                try:
                    db_cursor.execute("""
                        SELECT * FROM search_nav_data(%s, %s, %s, %s)
                    """, [search or '', page_size, cursor_id, 0])

                    results = db_cursor.fetchall()
                except Exception as db_error:
                    logger.error(f"PostgreSQL function error: {str(db_error)}")
                    # Fallback to ORM immediately
            ...
            nav_data = []
            total_count = results[0][10] if results and len(results[0]) > 10 else 0
            ...
            for row in results:
                try:
                    nav_item = {
                        'id': row[0],
                        'nav': row[1] or '',
                        'navDate': row[2].isoformat() if row[2] else None,
                        'createdAt': row[3].isoformat() if row[3] else None,
                        'updatedAt': row[4].isoformat() if row[4] else None,
                        'navFundName': row[5] or '',
                        'fundId': row[6] or 0,
                        'amcName': row[7] or '',
                        'amcId': row[8] or 0,
                        'schemeCode': row[9] or '',
            ...
            if len(nav_data) > page_size:
                next_cursor = nav_data[page_size - 1]['id']
                nav_data = nav_data[:page_size]
            ...
            # Always fallback to ORM method if PostgreSQL function fails
            return self.listing_fallback(request)

    @action(detail=False, methods=['GET'])
    def total_count(self, request):
        """
        Get total count using optimized PostgreSQL function
        """
        search = request.query_params.get('search', '').strip()

        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT get_nav_search_count(%s, %s)", [search or '', 0])
            ...
            # Fallback to ORM count with timeout protection
            try:
                queryset = self.get_queryset().select_related(
                    'navFundName',
                    'navFundName__fundAmcName'
                )
                ...
                # Use exists() check for large datasets
                if queryset.count() > 100000:
                    # For very large datasets, return approximate count
                    with connection.cursor() as cursor:
                        cursor.execute("""
                            SELECT reltuples::BIGINT AS estimate
                            FROM pg_class
                            WHERE relname = 'apis_navmodel'
                        """)
                        estimate = cursor.fetchone()[0]
                    return Response({
                        'code': 1,
                        'total_count': estimate,
                        'message': 'Approximate count retrieved'
                    })
                else:
                    ...
                    return Response({
                        'code': 1,
                        'total_count': count,
                        'message': 'Count retrieved successfully (Fallback)'
                    })
    ...
    def listing_fallback(self, request):
        """
        Optimized fallback method using Django ORM
        """
        user = request.user
        if not user.is_authenticated:
            return Response({'code': 0, 'message': "Token is invalid"}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        try:
            ...
            if len(results) > page_size:
                next_cursor = results[page_size - 1].id
            ...
            try:
                ...
                total_count = len(results)  # Fallback to current page count
    ...
    def get_nav_update_data(self, request, pk=None):
        """
        Get NAV update data using optimized PostgreSQL function
        """
        try:
            nav_id = int(pk)

            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM get_nav_update_data(%s)", [nav_id])
                result = cursor.fetchone()
            ...
            if result:
                data = {
                    'navId': result[0],
                    'nav': result[1] or '',
                    'navDate': result[2].isoformat() if result[2] else None,
                    'fundId': result[3],
                    'fundName': result[4] or '',
                    'schemeCode': result[5] or '',
                    'amcId': result[6],
                    'amcName': result[7] or ''
                }
                return Response({
                    'code': 1,
                    'data': data,
                    'message': 'NAV update data retrieved successfully'
                })
            else:
                return Response({
                    'code': 0,
            ...
            # Fallback to ORM
            try:
                instance = NavModel.objects.select_related(
                    'navFundName',
                    'navFundName__fundAmcName'
                ).get(id=pk, hideStatus=0)

                data = {
                    'navId': instance.id,
                    'nav': instance.nav or '',
                    'navDate': instance.navDate.isoformat() if instance.navDate else None,
                    'fundId': instance.navFundName.id if instance.navFundName else 0,
                    'fundName': instance.navFundName.fundName if instance.navFundName else '',
                    'schemeCode': instance.navFundName.schemeCode if instance.navFundName else '',
                    'amcId': instance.navFundName.fundAmcName.id if instance.navFundName and instance.navFundName.fundAmcName else 0,
                    'amcName': instance.navFundName.fundAmcName.amcName if instance.navFundName and instance.navFundName.fundAmcName else ''
                }
                return Response({
                    'code': 1,
                    'data': data,
                    'message': 'NAV update data retrieved successfully (Fallback)'
                })
                ...
                return Response({
                    'code': 0,
                    'message': f'Error retrieving NAV update data: {str(fallback_error)}'
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    @action(detail=False, methods=['GET'])
    def funds_by_amc(self, request):
        """
        Get funds by AMC using optimized function
        """
        amc_id = request.query_params.get('amc_id')
        if not amc_id:
            return Response({
        ...
        try:
            ...
            try:
                ...
                logger.error(f"PostgreSQL function error in funds_by_amc: {str(db_error)}")
                # Fallback to ORM
                funds = FundModel.objects.filter(
                    fundAmcName_id=amc_id,
                    hideStatus=0
                ).values('id', 'fundName', 'schemeCode').order_by('fundName')

                return Response({
                    'code': 1,
                    'data': list(funds),
                    'message': 'Funds retrieved successfully (Fallback)'
                })
