...
export class FormsService {
  ...
  constructor() {  
    ...
    this.lists = this.apiUrl + "forms/listing/";
    this.processing = this.apiUrl + "forms/";
    this.deletion = this.apiUrl + "forms/0/deletion/";
    this.amc = this.apiUrl + "amcEntry/";
    this.formTypes = this.apiUrl + "formType/";
  }
  ...
  listsForms(page: number, pageSize: number, search: string = ''): Observable<ApiResponse<any>> {
  ...
  getFormTypes() {
    return axios.get(this.formTypes, {
      headers: {
        "Authorization": `Bearer ${localStorage.getItem('access_token')}`
      }
    });
