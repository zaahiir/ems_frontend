...
import { industryAumCommonInterface} from '../../../interfaces/interfaces';
import Swal from 'sweetalert2';
import { Subject } from 'rxjs';
import { takeUntil, debounceTime, distinctUntilChanged } from 'rxjs/operators';

@Component({
  selector: 'app-list-industry-aum-entry',
  standalone: true,
  ...
  templateUrl: './list-industry-aum-entry.component.html',
  styleUrl: './list-industry-aum-entry.component.scss'
})

export class ListIndustryAumEntryComponent implements OnInit {
  icons = { cilPen, cilTrash, };
  tooltipEditText = 'Edit';
  tooltipDeleteText = 'Delete';

  industryAumInterfaceList: industryAumCommonInterface[] = [];
  pageRange: number[] = [];
  currentPage = 1;
  itemsPerPage = 10;
  searchTerm: string = '';
  ...
  constructor(private industryAumEntryService: IndustryAumEntryService) {}

  ngOnInit() {
    this.loadIndustryAumList();
    this.setupSearch();
  }
  ...
  async loadIndustryAumList() {
    if (this.isLoading) return;

    this.isLoading = true;
    this.industryAumEntryService.listsIndustryAumEntry(this.currentPage, this.itemsPerPage, this.searchTerm)
  ...
  handleResponse(response: any) {
    ...
    if (response && response.code === 1 && Array.isArray(response.data)) {
      this.industryAumInterfaceList = response.data;
  ...
  get paginatedAumList() {
    return this.industryAumInterfaceList;
  }
  ...
  async deleteIndAum(id: number) {
    ...
    if (result.isConfirmed) {
      try {
        const response = await this.industryAumEntryService.deleteIndustryAumEntry(id.toString());
        if (response.data.code === 1) {
          await Swal.fire('Deleted!', 'AUM has been deleted.', 'success');
          await this.loadIndustryAumList(); // Reload the list
        } else {
